...
class FaceDectectors(object):
	def __init__(self, wait=0.0):
		...
		DATA_DIR = abspath(join(dirname(__file__), 'known_faces'))
		...
		self.detection_model = face_detection_model('/usr/share/opencv/haarcascades/haarcascade_frontalface_default.xml')
	...
	def crop_face(self,image_data):
		...
		files = []
		face_files = self.detection_model.run(image_data)
		#print(face_files)
		return face_files
	...
	def image_callback(self,msg):
		...
		try:
			...
			for face in facelists:
				...
				encoding_size=len(face_recognition.face_encodings(face))
				if encoding_size>0 :
